...
% stimulus identity.
for sessionNo = 1:length(trials)
    % If Target labels are parsed
    if flag==1
        % Identify the row of the target location 
        row = trials(sessionNo).target.row;
        % Identifying the column
        col = trials(sessionNo).target.column;
        ...
        trial_label(1,sessionNo) = discrete_xy(ind,1);
        % If distractor labels are parsed
    elseif flag==2
        row = trials(sessionNo).distractors(2);
        col = trials(sessionNo).distractors(3);
        ind = discrete_xy(:,3)==row & discrete_xy(:,2)==col;
        trial_label(1,sessionNo) = discrete_xy(ind,1);
        % If target - distractor paired labels are parsed
    elseif flag==3
        % Identifying discrete target label
        row_t = trials(sessionNo).target.row;
        col_t = trials(sessionNo).target.column;
        ind_t = discrete_xy(:,3)==row_t & discrete_xy(:,2)==col_t;
        trial_label_tar = discrete_xy(ind_t,1);
        % Identifying discrete distractor label
        row_d = trials(sessionNo).distractors(2);
        col_d = trials(sessionNo).distractors(3);
        ...
        trial_label(1,sessionNo) = discrete_xy_joint(ind,3);
    ...
    elseif flag==4
        ...
        ind = discrete_xy_joint(:,1)==trial_label_tar & discrete_xy_joint(:,2)==trial_label_dist;
        trial_label(1,sessionNo) = discrete_xy_joint(ind,4);
    elseif flag==5
        % Identify the row of the target location  [1 2 2;2 2 3;3 2 4;4 3 2;5 3 4;6 4 2;7 4 3;8 4 4];
        rewarded = trials(sessionNo).reward;
        % Find the discrete target label from discrete_xy for the particular
        % row and column of the target.
        if ~isempty(rewarded)==1
            trial_label(1,sessionNo) = 1; %rewarded
        elseif isempty(rewarded)==1
            trial_label(1,sessionNo) = 0; %not rewarded
        end
        %ind = discrete_xy_reward(:,3)==n;
        % Assigning the discrete target label for trial i in trial_label
