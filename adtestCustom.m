function [H, pVal, ADStat, CV] = adtestCustom(x, normParameters, weibullParameters, varargin)
  
%% fitParameters
% fitParameters{1}='weibull';
% fitParameters{2}=1stValue;
% fitParameters{3}=2ndValue;

% fitParameters{1}='normal';
% fitParameters{2}=mean;
% fitParameters{3}=var;
    ...
    switch Distr
        case 'normal'
            if any(~isreal(x)) % This can occur when we do the test against the lognormal distribution
                ...
                z = normcdf(x, normParameters(1), normParameters(2));
                ADStat = ComputeADStat(z,n);
            end
        case 'exponential'
        ...
        case {'ev', 'extreme value'}
            if any(~isreal(x)) % This can occur when we do the test against the Weibull distribution
                ...
                %params = evfit(x);
                %z1 = evcdf(x,params(1), params(2));
                
                z = wblcdf(exp(x),weibullParameters(1), weibullParameters(2));
                ADStat = ComputeADStat(z,n);
            end
    end
