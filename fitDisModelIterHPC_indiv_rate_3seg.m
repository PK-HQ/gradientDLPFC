function bootstrapAICCont=fitDisModelIterHPC_indiv_rate_3seg(x,yActual,allBreaks,yLab,xLab,statLimits,statSpacing,...
...
%find 2 breaks
best2BreakPoints=find2Breaks(xFull(:),yFull(:),breakpoints,regionBoundaries,jitterBoundary,robustStr);
...
if ~isnan(xb1)==0 || ~isnan(xb2)==0
    ...
    n3seg=n3seg+1;
    edges = [min(xFull),xb1,xb2,max(xFull)];
...
switch validBreak
    case {3}
        ...
        mdlC=fitlm(x3Full,y3Full,'RobustOpts',robustStr);
    otherwise
        return
end
...
fprintf('Break@%.0f & %.0f: idx1=%.0f, idx2=%.0f, idx3=%.0f\n',xb1,xb2,numel(~isnan(idx1)),numel(~isnan(idx2)),numel(~isnan(idx3)))
x=[x1Full;x2Full;x3Full];
...
%save(['/home/svu/lsitpk/HPC_PK/data/em/gradient/shuffled/surrogate3seg_' MonkyInitial '_' regionStr '_' folderName '_' num2str(maxiters) 'i_' uniqueID '.mat'],'bootstrapR2Cont');
save(['/hpctmp/lsitpk/noncompiled/surrogate3seg_' MonkyInitial '_' regionStr '_' folderName '_' num2str(maxiters) 'i_' uniqueID '.mat'],'bootstrapR2Cont');
end
